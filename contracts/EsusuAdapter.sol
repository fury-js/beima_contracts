...
contract EsusuAdapter is OwnableService, ISavingsConfigSchema {
    ...
    */
    ...
    function CreateEsusu(uint256 groupId, uint256 depositAmount, uint256 payoutIntervalSeconds,uint256 startTimeInSeconds, address owner, uint256 maxMembers) external active onlyOwnerAndServiceContract {
        ...
        //  emit event
        emit CreateEsusuCycleEvent(block.timestamp, currentEsusuCycleId, depositAmount, owner, payoutIntervalSeconds,CurrencyEnum.Dai,"Dai Stablecoin",_esusuStorage.GetEsusuCycleState(currentEsusuCycleId));
    ...
    */
    ...
    function JoinEsusu(uint esusuCycleId, address member) public onlyOwnerAndServiceContract active {
       ...
       _esusuStorage.CreateMemberAddressToMemberCycleMapping(
        ...
        //  emit event 
        emit JoinEsusuCycleEvent(block.timestamp, member,TotalMembers, totalAmountDeposited,CycleId);
    }
    ...
    */
    ...
    function StartEsusuCycle(uint esusuCycleId) public onlyOwnerAndServiceContract active{
        ...
        require(block.timestamp > _esusuStorage.GetEsusuCycleStartTime(esusuCycleId),"Cycle can only be started when start time has elapsed");
        ...
        //  Update Esusu Cycle State, total cycle duration, total shares  and  cycle start time, 
        _esusuStorage.UpdateEsusuCycleDuringStart(CycleId,uint(CycleStateEnum.Active),toalCycleDuration,EsusuCycleTotalShares, block.timestamp);
        
        //  emit event 
        emit StartEsusuCycleEvent(block.timestamp, esusuCycleBalance, toalCycleDuration,
    ...
    */
    ...
    function DepricateContract(
        ...
        DepricateContractEvent(block.timestamp, owner, reason, yDaiSharesBalance);
